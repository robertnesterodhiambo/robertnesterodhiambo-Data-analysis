...
We are utilizing the `lapply` function and lists in R to import a set of packages. Vectors are stored within the `c()` function in R. Subsequently, we call this list and import all packages, where `lapply` takes package names as a list and runs the `library` function on each.

###   Packages use

1. **tidyverse**: Collection of R packages for data manipulation and visualization.
2. **plotly**: Interactive plotting library for creating web-based graphs.
3. **lubridate**: Simplifies date-time handling in R.
4. **readr**: Fast and friendly package for reading rectangular data.
5. **dplyr**: Data manipulation package for filtering, transforming, and summarizing data.
6. **zoo**: Provides tools for working with regular and irregular time series data.
7. **ggplot2**: Data visualization package based on the grammar of graphics.
8. **forecast**: Forecasting methods and tools for time series analysis.
9. **stats**: Core statistical functions and distributions in R.
10. **tseries**: Time series analysis and econometrics tools.
...
- **The data is imported using the `read_csv` function from the `readr` library. Our data includes a `Timestamp` column, which will be converted to a readable format using the code line `mutate(Timestamp = as.POSIXct(Timestamp, origin = "1970-01-01", tz = "UTC"))`.**

- This line interprets the `Timestamp` into a human-readable form. We set the format to UTC and provide a reference date, such as 1970-01-01, for conversion.

- Finally, we conclude the code by displaying the first 6 rows of the data.
...
weekly_rows <- filtered_data %>%
  ...
  slice_head(n = 1) %>%               # Select the first row from each group (week)
...
head(data)  # Output the cleaned and processed data frame
...
plt_hrsly <- ggplot(data_hourly, aes(x = Timestamp, y = Weighted_Price)) +
  ...
  scale_x_datetime(date_labels = "%d-%b %H:%M", date_breaks = "96 hours") +  # Set date breaks to every 96 hours (4 days)
  scale_color_manual(values = c("#F8766D"))

# Convert the ggplot2 plot to a plotly plot
plotly_hrsly <- ggplotly(plt_hrsly)
...
plt_hrsly2 <- ggplot(data_hourly, aes(x = Timestamp, y = Weighted_Price)) +
  geom_line(color = "#F8766D") +
  labs(title = "BTC Weighted_Price Price (USD) by Hours", x = "Timestamp", y = "Price") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
plotly_hrsly2 <- ggplotly(plt_hrsly2)
...
plot_prophet <- prophet_plot_components(model_fit, forecast)
plot_prophet
