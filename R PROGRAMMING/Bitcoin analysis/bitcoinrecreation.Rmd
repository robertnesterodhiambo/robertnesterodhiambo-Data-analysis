...
-   the lineinterprates the Timestamp to human readable form.
...
The summary statistics ddiplays the minimum valuue and maximum value which explains the range.
the 1st median meand and third quartile range are also in play they elaborate the interquatile ranges.
the summary also highlights the total number of missing variables ineach columns.
...
Sure, here's a table showing the equivalent R and Python code side by side, along with comments highlighting the differences.
...
-   From the code below we fill missing values in Open High Low and Close columns and replace NAs with previous value from the one above it.

-   This approach in the beginning helps in maintaining the continuity of data without losing important rows of the data other columns.
...
in this code below we utilise plotly function from plotly package to create an interactive plot.
...
The python output doesent distinguish the open closed and weighted_price clearlyy in the plot.
ALthtought ht plots interactively mathes that of R programming, we see a asuperiority of the R language in handling the visualizations in a more vasetile manner by creating multiple facaets for each column while still maintaining the same x and y axis.
...
Sure, here’s a side-by-side comparison of the R and Python code with comments highlighting the differences and similarities.
...
| library(zoo)                                                              | from sklearn.preprocessing import MinMaxScaler                        |
|                                                                           | from keras.models import Sequential                                   |
| # Data Preprocessing                                                      | from keras.layers import Dense, LSTM, Dropout, Activation              |
| data <- na.omit(data)                                                     |                                                                       |
| data$hour <- as.POSIXct(format(data$Timestamp, "%Y-%m-%d %H:00:00"), tz = "UTC")| data = data.dropna()                                              |
| data <- data[!duplicated(data$hour), ]                                    | data['hour'] = data['Timestamp'].dt.floor('H')                        |
| fill_missing <- function(values) {                                        | data = data.loc[~data['hour'].duplicated()]                           |
|   last_value <- NA                                                        |                                                                       |
|   filled_values <- numeric(length(values))                                |                                                                       |
|   for (i in seq_along(values)) {                                          |                                                                       |
|     if (is.na(values[i])) {                                               | fill_missing = lambda s: s.fillna(method='ffill')                     |
|       filled_values[i] <- last_value                                      |                                                                       |
|     } else {                                                              | data['Weighted_Price'] = data['Weighted_Price'].apply(fill_missing)   |
|       filled_values[i] <- values[i]                                       |                                                                       |
|       last_value <- values[i]                                             | split_date = '25-Jun-2018'                                            |
|     }                                                                     | data_train = data[data['hour'] <= split_date]                         |
|   }                                                                       | data_test = data[data['hour'] > split_date]                           |
|   return(filled_values)                                                   |                                                                       |
| }                                                                         | training_set = data_train['Weighted_Price'].values.reshape(-1, 1)      |
| data$Weighted_Price <- fill_missing(data$Weighted_Price)                  |                                                                       |
|                                                                           | sc = MinMaxScaler()                                                   |
| split_date <- as.POSIXct("2018-06-25", tz = "UTC")                        | training_set = sc.fit_transform(training_set)                         |
| data_train <- data[data$hour <= split_date, "Weighted_Price", drop = FALSE]| X_train = training_set[:-1]                                          |
| data_test <- data[data$hour > split_date, "Weighted_Price", drop = FALSE] | y_train = training_set[1:]                                            |
...
| scaled_train <- scale_data(data_train$Weighted_Price)                     | model = Sequential()                                                  |
| scaled_test <- scale_data(data_test$Weighted_Price)                       | model.add(LSTM(128, activation="sigmoid", input_shape=(1, 1)))        |
| training_set <- scaled_train$scaled_values                                | model.add(Dropout(0.2))                                               |
| testing_set <- scaled_test$scaled_values                                  | model.add(Dense(1))                                                   |
|                                                                           | model.compile(loss='mean_squared_error', optimizer='adam')             |
| X_train <- training_set[-length(training_set)]                            | model.fit(X_train, y_train, epochs=100, batch_size=50, verbose=2)      |
...
### Comments on Differences
In R here we used self made functions and in python libraries were utilised. Utilising libraries in r would have meant creating virtual python environament to utilise Keras libraries. This would have meant utilising same pyhton code again.
