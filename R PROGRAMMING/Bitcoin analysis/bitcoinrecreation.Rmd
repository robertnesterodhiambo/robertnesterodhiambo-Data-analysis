...
| data \<- data[!duplicated(data$hour), ]                         | data['hour'] = data['Timestamp'].dt.floor('H')                |          
 | fill_missing <- function(values) {                             | fill_missing = lambda s: s.fillna(method='ffill')            |           
 |   last_value <- NA                                            | data['Weighted_Price'] = data['Weighted_Price'].apply(fill_missing) |     
 |   filled_values <- numeric(length(values))                    |                                                               |           
 |   for (i in seq_along(values)) {                              | split_date = '25-Jun-2018'                                    |           
 |     if (is.na(values[i])) {                                   | data_train = data[data['hour'] <= split_date]                  |          
 |       filled_values[i] <- last_value                          | data_test = data[data['hour'] > split_date]                    |          
 |     } else {                                                  |                                                               |           
 ...
 |                                                               | sc = MinMaxScaler()                                           |           
 | split_date <- as.POSIXct("2018-06-25", tz = "UTC")            | training_set = data_train['Weighted_Price'].values.reshape(-1, 1) |       
 | data_train <- data[data$hour \<= split_date, "Weighted_Price", drop = FALSE]                                                              |                                                           |
| data_test \<- data[data$hour > split_date, "Weighted_Price", drop = FALSE]  | X_train = training_set[:-1]                                | 
 |                                                               | y_train = training_set[1:]                                   |            
 | scale_data <- function(values) {                              | X_train = X_train.reshape(len(X_train), 1, 1)                 |           
...
In R here we used self made functions and in python libraries were utilised.
Utilising libraries in r would have meant creating virtual python environament to utilise Keras libraries.
This would have meant utilising same python code again.
