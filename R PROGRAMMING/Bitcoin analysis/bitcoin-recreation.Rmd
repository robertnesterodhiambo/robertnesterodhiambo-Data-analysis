...
train(model, X_train, y_train, epochs = 10, lr = 0.001)  # Train the model
...
```{r}
# Define the modified forward function to return the hidden state
forward <- function(model, x) {
  h_prev <- rep(0, model$units)  # Initialize previous hidden state to zero
  c_prev <- rep(0, model$units)  # Initialize previous cell state to zero
  for (t in 1:dim(x)[1]) {  # Iterate through each time step
    xt <- x[t, , , drop = FALSE]  # Extract current time step input
    f_t <- sigmoid(xt %*% model$Wf + model$bf)  # Forget gate activation
    i_t <- sigmoid(xt %*% model$Wi + model$bi)  # Input gate activation
    c_tilde_t <- tanh_act(xt %*% model$Wc + model$bc)  # Candidate cell state
    c_t <- f_t * c_prev + i_t * c_tilde_t  # Update cell state
    o_t <- sigmoid(xt %*% model$Wo + model$bo)  # Output gate activation
    h_t <- o_t * tanh_act(c_t)  # Update hidden state
    h_prev <- h_t  # Update previous hidden state for the next time step
    c_prev <- c_t  # Update previous cell state for the next time step
  }
  y <- h_t %*% model$Wy + model$by  # Compute the output
  return(list(y = y, h = h_t))  # Return both the output and hidden state
}

# Define the prediction function to generate predictions for the input data
predict <- function(model, X) {
  predictions <- numeric(dim(X)[1])  # Initialize a vector to store predictions
  for (i in 1:dim(X)[1]) {  # Iterate through each input sample
    output <- forward(model, X[i, , , drop = FALSE])  # Perform forward pass
    predictions[i] <- output$y  # Store the output in the predictions vector
  }
...
# Function to reverse the scaling of data to its original scale
unscale_data <- function(scaled_values, mean_val, std_val) {
  unscaled_values <- scaled_values * std_val + mean_val  # Reverse the scaling operation
  return(unscaled_values)  # Return the unscaled values
}

# Unscale predictions and actual values using the mean and standard deviation
mean_val <- scaled_data$mean  # Mean value used for scaling
std_val <- scaled_data$std  # Standard deviation used for scaling
unscaled_predictions <- unscale_data(predictions, mean_val, std_val)  # Unscale predictions
unscaled_actual <- unscale_data(y_train, mean_val, std_val)  # Unscale actual values

# Check lengths to ensure they match for plotting
length(unscaled_predictions)
length(unscaled_actual)

# Ensure the lengths match for plotting by taking the minimum length
plot_length <- min(length(unscaled_predictions), length(unscaled_actual), length(data$hour) - 1)

# Adjust unscaled predictions and actuals to match the plot length
unscaled_predictions <- unscaled_predictions[1:plot_length]  # Trim predictions to plot length
unscaled_actual <- unscaled_actual[1:plot_length]  # Trim actual values to plot length
plot_hours <- data$hour[2:(plot_length + 1)]  # Use 2nd to (length+1) to match y_train, since y_train skips the first hour
