...
Let’s assume you have a dataset named `sales_by_states` with columns `State`, `Sales_Amount`, and `Average_Price`:
...
ggplot(sales_by_states, aes(x = Sales_Amount, y = Average_Price, size = Sales_Amount)) +
...
### Notes:
1. Replace the simulated datasets with the actual ones (`sales_data`, `sales_by_states`, etc.).
2. Make sure the column names in your actual dataset match what is used in the code.
3. For the **Choropleth map**, ensure your dataset contains state names and sales data, which can be merged with `map_data("state")`.

This R code should cover the core visualizations you need to create for Patriots Corporation's data analysis.